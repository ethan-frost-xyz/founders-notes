...
Parent-tier embeddings power Librarian hybrid search (`retrieval_orchestrator` + parent-tier chunks; variant searches run concurrently) — not a repo-wide vector DB. Legacy `search_vault_parent` remains in `services/telegram/bot/tools/vault.py` for tests/harness only. See [`docs/retrieval.md`](../../docs/retrieval.md).
