...
Parking lot for follow-ups out of scope for the shipped stack. Organized as **Next (pick one cluster → new plan)**, **Decided / won't do**, and **Shipped (reference)** at the bottom. When implementing, pull one cluster into a new focused `.cursor/plans/*.plan.md`. Contracts and decisions for the shipped Telegram agent live in [`docs/telegram-vault-agent.md`](docs/telegram-vault-agent.md); removed master plan `telegram_rag_bot_v0` deferred items are consolidated below.
