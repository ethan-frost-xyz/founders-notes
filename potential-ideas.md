...
- **Default prod retrieval slug** — Seed `deepseek/deepseek-v4-flash` for `retrieval_model` on mini install (or Groq via [janitor.md](docs/janitor.md#model-tuning-playbook)); tune live with Telegram `/setmodel retrieval …` and confirm via `/settings`.
