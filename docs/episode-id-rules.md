...
Padding width is `EPISODE_NUMBER_WIDTH = 4` in `ingestion/episode_ids.py` (supports ep-0001 … ep-9999). Plain alphabetical sort of folders then matches episode order.
...
All path helpers live in `ingestion/paths.py` (`folder_name`, `content_filename`, `transcript_path`, `notes_file_path`, etc.). `ingestion/verify.py` enforces both rules on every run.
...
Generated by datapoint workflow (manual or promoted draft). Sections: `## Expanded datapoints` with quote + takeaway per bullet.
...
| `catalog/post-mapping-review.jsonl` | Auto-generated by `organize_posts_from_csv.py` (medium-confidence matches) |

## CLI episode selectors

| Script | Flag | Value | Notes |
|--------|------|-------|-------|
| `fetch_transcripts.py` | `--id` | `ep-0200` | Canonical padded id; legacy `ep-200` also resolves |
| `scaffold_notes.py` | `--id` | `ep-0200` | Single episode; also `--from` / `--to` (integers), `--missing`, `--next` |
| `expand_datapoints.py` | `--id` | `ep-0200` | Required; optional `--prompt` |
| `expand_datapoints_llm.py` | `--id`, `--from` / `--to`, `--missing-expanded`, `--promote` | — | Always with `--dry-run` or `--apply`; see `docs/datapoint-workflow.md` |
| `assign_post_manual.py` | `--episode` | `148` | Integer `episode_number`, **not** `ep-0148` |

Path helpers: `ingestion/paths.py`.
