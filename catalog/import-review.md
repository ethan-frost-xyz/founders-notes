...
**189 notes** were bulk-imported once (`source: apple_notes_import` in frontmatter). Workflow is vault-native only — edit `.notes.md` in git. One-shot importer removed (git history before `2fb9d22`).
...
Attribute skips `post_kind: article` units; assign bodies manually for native X articles.

## Native X articles — needs manual body paste

These episodes were published as X articles. Auto-attribute must not own them. Paste full article text into `import/*.txt` (gitignored) and run `assign_post_manual.py`.
...
- **ep-0021** — notes still contain `- XYZ` placeholder (no Apple Notes export); add real datapoints when listened
